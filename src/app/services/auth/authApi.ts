...
export const getTokens = async (email: string, password: string) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(data.message || data.detail || `Token retrieval failed with status: ${response.status}`);
    }
...
export const refreshToken = async (refreshToken: string) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(data.detail || `Token refresh failed with status: ${response.status}`);
    }
