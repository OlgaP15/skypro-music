  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        throw new Error(data.message || 'Пользователь с таким email уже существует');
      }
...
export const getTokens = async (email: string, password: string) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.detail || data.message || `Ошибка получения токена: ${response.status}`);
    }
...
export const refreshToken = async (refreshToken: string) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.detail || `Ошибка обновления токена: ${response.status}`);
    }
